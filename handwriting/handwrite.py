...
def main():
    with open(os.path.join('handwriting/data', 'translation.pkl'), 'rb') as file:
    ...
    with tf.Session(config=config) as sess:
        saver = tf.train.import_meta_graph('handwriting/' + args.model_path + '.meta')
        saver.restore(sess, 'handwriting/' + args.model_path)
